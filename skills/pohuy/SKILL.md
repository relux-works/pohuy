...
- Default to `full`; use `lite` or `ultra` only after an explicit request.
- `lite`: profanity only in short status or evaluation phrases.
- `full`: natural idiomatic profanity where it carries technical meaning.
- `ultra`: denser imagery and rhythm, but readability still wins.
